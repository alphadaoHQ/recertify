...
export function Header({ isDarkMode, toggleTheme }: HeaderProps) {
  return (
    <header
      className={`sticky top-0 z-50 backdrop-blur-xl border-b transition-all duration-500 px-3 sm:px-4 py-2.5 sm:py-3.5 ${isDarkMode
      ...
      <div className="flex items-center justify-between max-w-md mx-auto w-full border-rounded-sm">
        ...
        <div className="flex items-center gap-2 sm:gap-3 group min-w-0">
          <div className="relative shrink-0">
            {/* Glow effect */}
            <div
              className={`absolute inset-0 rounded-xl blur-lg opacity-60 transition-all duration-500 group-hover:opacity-80 ${isDarkMode
            ...
            {/* Logo Container */}
            <div
              className={`relative rounded-xl p-1.5 sm:p-2 transition-all duration-300 group-hover:scale-105 ${isDarkMode
          ...
          <div className="flex flex-col min-w-0">
            <div className="flex items-center gap-1.5 flex-wrap">
              <Sparkles
                className={`w-3 h-3 sm:w-3.5 sm:h-3.5 transition-colors duration-300 ${isDarkMode ? "text-purple-400" : "text-purple-600"
                  }`}
              />
            </div>
            <p
              className={`text-[10px] sm:text-xs font-medium transition-colors duration-300 ${isDarkMode ? "text-purple-300/80" : "text-black/70"
                }`}
            >
              Web3 Onboarding
            </p>
        ...
        <div className="flex items-center gap-1.5 sm:gap-2.5 shrink-0">
          ...
          <button
            type="button"
            onClick={toggleTheme}
            className={`group relative p-1.5 sm:p-2.5 rounded-xl transition-all duration-300 hover:scale-110 active:scale-95 ${isDarkMode
          ...
          >
            <div
              className={`absolute inset-0 rounded-xl opacity-0 group-hover:opacity-100 transition-opacity duration-300 ${isDarkMode
          ...
          {/* Wallet Connect */}
          <div
            className={`relative rounded-xl transition-all duration-300 hover:scale-105 ${isDarkMode
              ? "ring-1 ring-purple-800/40 shadow-lg shadow-purple-950/30"
              : "ring-1 ring-purple-200/60 shadow-md shadow-purple-100"
              }`}
          >
            <div
              className={`absolute inset-0 rounded-xl transition-opacity duration-300 ${isDarkMode
